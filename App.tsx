...
function FieldApp() {
  ...
  const [email, setEmail] = useState('ali.kaya@fieldops.test')
  const [password, setPassword] = useState('FieldOps!2026')
  ...
  const load = useCallback(async (auth: string) => {
    const [user, jobRes, noteRes] = await Promise.all([
    ...
    <SafeAreaView style={styles.screen} edges={['top']}>
      ...
      >
        ...
        {(tab === 'today' || tab === 'jobs') && (
          <>
            ...
            {jobs.map(job => (
              <Pressable key={job.id} style={[styles.card, open?.id === job.id && styles.cardOpen]} onPress={() => setOpen(job)}>
      ...
      {open && tab !== 'me' && (
        <View style={[styles.sheet, { bottom: 56 + insets.bottom }]}>
          ...
          <Pressable onPress={() => setOpen(null)} style={{ marginTop: 12 }}>
